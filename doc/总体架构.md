...
| SerialPortConnectionService | `src/SerialPortConnection/` | 连接域：连接参数随请求传入并映射为 HAL 打开参数、经 HAL 打开串口、创建/销毁 Connection、映射与状态回写 | 已实现 |
...
| 预设组合管理 UI（设置项图形入口：列表 + 增改删向导） | 已实现 |
| 配置子节点 inline 连接、设备级连接按钮迁移（hasConfigs） | 未实现（当前里程碑） |
