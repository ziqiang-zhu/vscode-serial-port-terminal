...
6. 连接参数由调用方随 `connect(device, config)` 传入，ConnectionService 不依赖配置存储（扩展不做启动自动连接）。
...
- 连接建立后，ConnectionService 通过工厂注册默认 Consumer（SerialPortTerminal）；依附型 Consumer 由主 Consumer 经 `host.addConsumer` 接入；
- 多 Consumer 注册已支持；依附型 Consumer 总是依附于主 Consumer（由主 Consumer 创建并托管生命周期），不提供独立的管理界面。数据的转义、解析与显示加工由 Consumer 内部经数据处理类完成，不属于 Consumer 本体；
