...
- 💾 **日志保存** —— 终端标题栏「保存/暂停/停止」，有数据才建文件，文件名精确到秒，剥离 ANSI 转义序列（颜色码等），停止时提示保存路径
...
- 多 Consumer 二级菜单管理
- 启动自动恢复（上次设备与配置）
- 日志增强：保存/暂停/停止快捷键、按大小分割、时间戳、更多配置（分割大小、时间戳开关/间隔、编码）
- 命令面板：为所有命令添加 `Serial Port Terminal` 前缀（category），便于区分
